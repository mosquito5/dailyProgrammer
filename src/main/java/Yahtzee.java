...
public class Yahtzee {
     ...
     * You are given a Yahtzee dice roll, represented as a sorted list of 5 integers,
     * each of which is between 1 and 6 inclusive.
     ...
     * For instance, consider the roll [2, 3, 5, 5, 6]. If you scored this as 1's,
     * the score would be 0, since there are no 1's in the roll.
    ...
    public int yahtzee_upper(int[] dice) {
        HashMap<Integer, Integer> score = new HashMap<>();
        for (int diceVal : dice) {
            if (score.containsKey(diceVal)) {
                int value = score.get(diceVal);
                value += diceVal;
                score.put(diceVal, value);
            } else
                score.put(diceVal, diceVal);
        }
